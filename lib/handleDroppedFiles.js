...
export default function onDropFile(config) {
  return function onDropFileInner(selection, files, refs) {
    const getEditorState = refs.getEditorState;
    const setEditorState = refs.setEditorState;
    ...
    if (handleUpload) {
      ...
      const data = { files: [], formData: formData };
      ...
      readFiles(data.files).then(function(placeholders) {
        ...
        // Perform upload
        handleUpload(data, function(uploadedFiles, options) {
          options = options || {};

          const retainSrc = options.retainSrc || false;

          // Success, remove 'progress' and 'src'
          var newEditorState = getEditorState();
          uploadedFiles.forEach(function(file) {
            const blocks = getBlocksWhereEntityData(state, function(block) { return block.src === file.src && block.progress !== undefined; });
            if (blocks.size) {
          ...
          var newEditorState = getEditorState();
          placeholders.forEach(function(placeholder) {
            const blocks = getBlocksWhereEntityData(newEditorState, function(p) { return p.src === placeholder.src && p.progress !== undefined; });
            if (blocks.size) {
